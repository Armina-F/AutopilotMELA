...
def to_level(series: pd.Series, split_1: float, split_2: float) -> pd.Series:
    ...
    levels.loc[(values > split_1) & (values <= split_2)] = "mid"
...
def write_manual_abstraction(learning_set: str, configuration: str) -> None:
    input_file_path = f"Data/Autopilot/Learning set/Manual/{learning_set}/{configuration}.csv"
    output_file_path = f"Data/Autopilot/Abstraction/Manual/{learning_set}/{configuration}.csv"

    input_path = project_path(input_file_path)
    output_path = project_path(output_file_path)

    if not input_path.exists():
        print(f"Skipping missing Manual learning set: {input_file_path}")
        return
